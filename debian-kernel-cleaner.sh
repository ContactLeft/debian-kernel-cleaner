...
dpkg -l 'linux-*' | grep -e linux-image-[0-9] -e linux-headers-[0-9] | grep ^ii | grep -v $CURRENTKERN_VERSION | grep -v $NEWESTKERN_VERSION | sed -r 's/ +/ /g' | cut -d' ' -f2 | xargs apt-get -y purge | update-grub2
